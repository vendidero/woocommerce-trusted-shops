...
class WC_Trusted_Shops_Template_Hooks {
	...
	public function insert_rich_snippets() {
		...
		if ( in_array( 'category', $this->base->get_rich_snippets_locations(), true ) ) {
			if ( is_product_category() ) {
				$insert = true;
			}
		}

		if ( in_array( 'home', $this->base->get_rich_snippets_locations(), true ) ) {
			if ( function_exists( 'is_shop' ) && is_shop() ) {
				$insert = true;
			}
		}

		if ( in_array( 'product', $this->base->get_rich_snippets_locations(), true ) ) {
			if ( is_product() ) {
				$insert = true;
			}
		}
	...
	public function cancel_review_reminder_check() {
		if ( isset( $_GET['disable-review-reminder'] ) && isset( $_GET['order-id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended

			$order_id = absint( $_GET['order-id'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$code     = wc_clean( wp_unslash( $_GET['disable-review-reminder'] ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			if ( ! empty( $code ) && ! empty( $order_id ) ) {
				...
				while ( $order_query->have_posts() ) {
					...
					if ( $order ) {
						...
						wp_die( wp_kses_post( sprintf( _x( 'Your review reminder e-mail has been cancelled successfully. Return to %s.', 'trusted-shops', 'woocommerce-trusted-shops' ), '<a href="' . esc_url( get_site_url() ) . '">' . esc_html_x( 'Home', 'trusted-shops', 'woocommerce-trusted-shops' ) . '</a>' ) ) );
					}
				}
			}
		}
	...
	public function email_cancel_review_reminder( $order, $sent_to_admin, $plain_text ) {
		...
		WC_GZD_Cache_Helper::maybe_flush_cache(
			'db',
			array(
				'cache_type' => 'meta',
				'meta_type'  => 'post',
				'meta_key'   => 'ts_review_reminder_opted_in', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
			)
		);
	...
	public function update_order_meta( $order_id ) {
		...
		if ( isset( $_POST['review_reminder'] ) || ! $checkbox || ( $checkbox && ! $checkbox->is_enabled() ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
	...
	public function review_reminder_checkbox() {
		...
		wc_gzd_register_legal_checkbox(
			...
			array(
				...
				'label'                => _x( 'Yes, I would like to be reminded via e-mail after {days} day(s) to review my order. I am able to cancel the reminder at any time by clicking on the "cancel review reminder" link within the order confirmation.', 'trusted-shops', 'woocommerce-trusted-shops' ),
				'label_args'           => array( '{days}' => $this->base->review_reminder_days ),
				'hide_input'           => false,
				'is_enabled'           => false,
				'is_mandatory'         => false,
				'error_message'        => _x( 'Please allow us to send a review reminder by e-mail.', 'trusted-shops', 'woocommerce-trusted-shops' ),
				'priority'             => 6,
				'is_core'              => true,
				'admin_name'           => _x( 'Review reminder', 'trusted-shops', 'woocommerce-trusted-shops' ),
				'admin_desc'           => _x( 'Asks the customer to receive a Trusted Shops review reminder.', 'trusted-shops', 'woocommerce-trusted-shops' ),
	...
	public function set_product_widget_template( $template ) {
		...
		if ( in_array( $template, array( 'single-product/rating.php' ), true ) ) {
	...
	public function template_review_sticker( $template ) {
		wc_get_template(
			'trusted-shops/review-sticker.php',
			array(
				'plugin'  => $this->base,

...
if ( ! class_exists( 'WooCommerce_Trusted_Shops' ) ) :
	...
	final class WooCommerce_Trusted_Shops {
		...
		public function __construct() {
			...
			$this->trusted_shops = new WC_Trusted_Shops(
				$this,
				array(
		...
		public function add_notices() {
			...
			if ( 1 === (int) get_option( '_wc_ts_needs_update' ) ) {
				if ( current_user_can( 'manage_woocommerce' ) ) {
					wp_enqueue_style( 'woocommerce-activation', plugins_url( '/assets/css/activation.css', WC_PLUGIN_FILE ), array(), WC_TRUSTED_SHOPS_VERSION );
					wp_enqueue_style( 'woocommerce-ts-activation', plugins_url( '/assets/css/activation.css', WC_TRUSTED_SHOPS_PLUGIN_FILE ), array(), WC_TRUSTED_SHOPS_VERSION );
		...
		<th scope="row" class="titledesc">
			<span class="wc-gzd-label-wrap"><?php echo esc_html( $value['title'] ); ?> <?php echo wp_kses_post( $tooltip_html ); ?></span>
		</th>
		<td class="forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?>">
			<a href="#" class="woocommerce-ts-input-toggle-trigger">
				<span id="<?php echo esc_attr( $value['id'] ); ?>-toggle" class="woocommerce-ts-input-toggle woocommerce-input-toggle woocommerce-input-toggle--<?php echo ( ( 'yes' === $option_value ) ? 'enabled' : 'disabled' ); ?>"><?php echo ( ( 'yes' === $option_value ) ? esc_html_x( 'Yes', 'trusted-shops', 'woocommerce-trusted-shops' ) : esc_html_x( 'No', 'trusted-shops', 'woocommerce-trusted-shops' ) ); ?></span>
			</a>
			<input
		...
		public function admin_footer_text( $footer_text ) {
			...
			if ( isset( $_GET['tab'] ) && 'trusted-shops' === $_GET['tab'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				$footer_text = wp_kses_post( sprintf( _x( 'If the <strong>App</strong> helped you, please leave a %1$s&#9733;&#9733;&#9733;&#9733;&#9733;%2$s in the WordPress plugin repository.', 'trusted-shops', 'woocommerce-trusted-shops' ), '<a href="https://wordpress.org/support/view/plugin-reviews/woocommerce-trusted-shops?rate=5#postform" target="_blank" class="wc-rating-link">', '</a>' ) );
			}
		...
		public function setup_compatibility() {
			$plugins = apply_filters(
				'woocommerce_ts_compatibilities',
				array(
 ...
 */
function WC_trusted_shops() { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName.FunctionNameInvalid

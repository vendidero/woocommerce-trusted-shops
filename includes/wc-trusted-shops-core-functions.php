...
function wc_ts_get_crud_data( $object, $key ) {
    ...
    if ( 'id' === $key && is_callable( array( $object, 'is_type' ) ) && $object->is_type( 'variation' ) && ! WC_TS_Dependencies::instance()->woocommerce_version_supports_crud() ) {
        $key = 'variation_id';
    } else if ( 'parent' === $key && is_callable( array( $object, 'is_type' ) ) && $object->is_type( 'variation' ) && ! WC_TS_Dependencies::instance()->woocommerce_version_supports_crud() ) {
    ...
    } else if ( WC_TS_Dependencies::instance()->woocommerce_version_supports_crud() ) {
        if ( substr( $key, 0, 1 ) !== '_' )
            $key = '_' . $key;

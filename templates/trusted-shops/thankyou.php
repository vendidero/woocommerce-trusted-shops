...
<div id="trustedShopsCheckout" style="display: none;">
    ...
    <span id="tsCheckoutOrderPaymentType"><?php echo WC_trusted_shops()->trusted_shops->get_payment_gateway( wc_ts_get_crud_data( $order, 'payment_method' ) );?></span>
	...
	<?php if ( WC_trusted_shops()->trusted_shops->is_product_reviews_enabled() ) : ?>
		<?php foreach( $order->get_items() as $item_id => $item ) :
			...
			if ( has_post_thumbnail( wc_ts_get_crud_data( $product, 'id' ) ) )
				$image = wp_get_attachment_image_src( get_post_thumbnail_id( wc_ts_get_crud_data( $product, 'id' ) ), apply_filters( 'single_product_large_thumbnail_size', 'shop_single' ) );

			?>
            <span class="tsCheckoutProductItem">
				<span class="tsCheckoutProductUrl"><?php echo get_permalink( wc_ts_get_crud_data( $product, 'id' ) ); ?></span>

...
copy_dest_files() {
	...
	rsync ./ "$2"/ --recursive --delete --delete-excluded \
		...
		--exclude=auth.json \
		--exclude=woocommerce-trusted-shops.zip \
		--exclude="zip-file/"
	status "Done copying files!"
	cd "$CURRENT_DIR" || exit
}

status "💃 Time to release WooCommerce Trusted Shops 🕺"
...
cd zip-file
zip -r ../woocommerce-trusted-shops.zip ./
cd ..
rm -r zip-file

success "Done. You've built WooCommerce Trusted Shops! 🎉"